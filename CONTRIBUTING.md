...
```bash
git clone https://github.com/floatpane/go-keybind.git
cd go-keybind
go mod tidy
```
...
```
feat(key): add support for numpad key names
fix(config): handle symlinked config directories in Load
docs: document modifier alias normalization
```
...
- A minimal reproducer (a `Parse` / `Load` call, the key string or JSON that triggers the bug, or a failing test)
